  ...
  useEffect(() => {
    const initSession = async () => {
      try {
        ...
        // Check if user has a valid session (set by callback route)
        const { data: { session } } = await client.auth.getSession();
        setHasSession(!!session);

        if (!session) {
          setErrorMessage("Ingen aktiv session. Begär en ny återställningslänk.");
        }
      } catch (e: unknown) {
        ...
        <div className="w-full max-w-md py-10">
          <Card className="w-full">
            <CardHeader className="text-center pb-2">
              ...
              <CardTitle className="text-2xl font-semibold">Session saknas</CardTitle>
              <CardDescription className="text-base">
                Din session har utgått. Begär en ny återställningslänk.
              </CardDescription>
            </CardHeader>
              ...
              {!successMessage && (
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <Button
                    ...
                    disabled={isSubmitting || !password || !supabase}
                  >
                    {isSubmitting ? (
