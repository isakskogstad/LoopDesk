    ...
    async start(controller) {
      ...
      try {
        ...
        await page.addInitScript(() => {
          ...
          // Patch to handle Angular fixLink bug - prevents crash when replaceAll is called on undefined
          // This patches the String prototype to safely handle undefined 'this' context
          ...
          // Global error handler to catch and suppress Angular fixLink errors
          window.addEventListener('error', (event) => {
            if (event.message && event.message.includes("Cannot read properties of undefined (reading 'replaceAll')")) {
              console.warn('[Suppressed] Angular fixLink error:', event.message);
          ...
          window.addEventListener('unhandledrejection', (event) => {
            if (event.reason && String(event.reason).includes('replaceAll')) {
              console.warn('[Suppressed] Unhandled rejection with replaceAll:', event.reason);
              event.preventDefault();
            }
          });
        });
...
async function fetchDetailText(
  ...
  await detailPage.addInitScript(() => {
    ...
    // Global error handler to suppress Angular errors
    window.addEventListener('error', (event) => {
      if (event.message && (
