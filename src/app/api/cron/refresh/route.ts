...
import { prisma, createTitleHash } from "@/lib/db";
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    await prisma.feedCache.deleteMany({
      where: {
        expiresAt: { lt: new Date(now.getTime() - CACHE_TTL_MS * 2) },
      },
    });

    const duration = Date.now() - startTime;

    console.log(`Cron: Completed in ${duration}ms. ${sortedItems.length} unique items from ${allFeeds.length} feeds.`);
    if (errors.length > 0) {
      console.log(`Cron: ${errors.length} errors:`, errors.slice(0, 5));
    }
    ...
    return NextResponse.json({
      ...
      itemCount: sortedItems.length,
      articlesStored: articlesToInsert.length,
    ...
    return NextResponse.json(
      { error: "Failed to refresh feeds", details: error instanceof Error ? error.message : "Unknown" },
      { status: 500 }
    );
  }
}
